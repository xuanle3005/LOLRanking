...
def game_exists(conn, game_id):
    sql = """
        SELECT COUNT(*) as count FROM game_data WHERE game_id = %s;
    """
    result = execute_read(conn, sql, {"game_id": game_id}, only_one=True)
...
def process(game_id, platform_game_id):
    ...
    try:
        ...
        conn.commit()
        os.unlink(f"{directory}/{platform_game_id}.json")
...
def populate_game_data(conn, game_id, platform_game_id):
    with open(f"{directory}/{platform_game_id}.json", "r") as f:
        ...
        for event in game_events:
            if event['eventType'] == 'game_info':
                ...
                event_data = {
                    ...
                    "game_date": datetime.datetime.fromisoformat(event['eventTime']).date(),
                }
...
if __name__ == '__main__':
    try:
        ...
        directory = "games-data"
        if not os.path.exists(directory):
            os.makedirs(directory)
        exc_res, res = get_game_ids(conn)
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = []
            for game_id, platform_game_id in res:
                futures.append(executor.submit(process, game_id, platform_game_id))
